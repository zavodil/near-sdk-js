...
static void log_str(char *s) {
    log_utf8(strlen(s), (uint64_t)s);
}

static void long_str(long long unsigned num) {
    char b[1000];
    sprintf( b, "Formatted data:  %llu", num );
    log_utf8(strlen(b), (uint64_t)b);
}
...
static void call_js_contract_internal_my(char *contract, char *method, char *args) {
    char *in;
    uint64_t contract_len = strlen(contract);
    uint64_t method_len = strlen(method);
    uint64_t args_len = 0;
    size_t in_len;
    ...
    jsvm_call(contract_len, contract, method_len, method, in_len, in, false);
...
//char *contract_id = "nft3.jswm.testnet";
char *contract_id = "nft.js-vm.near";


void nft_metadata () __attribute__((export_name("nft_metadata"))) {
    call_js_contract_internal_my(contract_id, "nft_metadata", "'{}'");
}

void nft_token () __attribute__((export_name("nft_token"))) {
    call_js_contract_internal_my(contract_id, "nft_token", "");
}

void nft_token_metadata () __attribute__((export_name("nft_token_metadata"))) {
    call_js_contract_internal_my(contract_id, "nft_token_metadata", "");
}

void nft_tokens_for_owner () __attribute__((export_name("nft_tokens_for_owner"))) {
    call_js_contract_internal_my(contract_id, "nft_tokens_for_owner", "");
}

void nft_tokens_for_owner_set () __attribute__((export_name("nft_tokens_for_owner_set"))) {
    call_js_contract_internal_my(contract_id, "nft_tokens_for_owner_set", "");
}

void nft_supply_for_owner () __attribute__((export_name("nft_supply_for_owner"))) {
    call_js_contract_internal_my(contract_id, "nft_supply_for_owner", "");
}
